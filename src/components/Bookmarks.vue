...
export default {
  ...
  computed: {
    titleErrors () {
      ...
      !this.$v.addLink.title.required && errors.push('Введите Подпись')
      return errors
    },
    urlErrors () {
      const errors = []
      if (!this.$v.addLink.url.$dirty) return errors
      !this.$v.addLink.url.required && errors.push('Введите ссылку')
      return errors
    },
    updateTitleErrors () {
      const errors = []
      if (!this.$v.editLink.title.$dirty) return errors
      !this.$v.editLink.title.required && errors.push('Введите Подпись')
      return errors
    },
    updateUrlErrors () {
      const errors = []
      if (!this.$v.editLink.url.$dirty) return errors
      !this.$v.editLink.url.required && errors.push('Введите ссылку')
      return errors
    },
